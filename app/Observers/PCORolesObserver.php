...
class PCORolesObserver
{
    public function creating(PCORoles $pco_role){
        $pco_role->created_by = session()->get('user_id');
        $pco_role->updated_by = session()->get('user_id');
        $pco_role->created_at = date('Y-m-d H:i:s');
        $pco_role->updated_at = date('Y-m-d H:i:s');
    }
    ...
    public function updating(PCORoles $pco_role){
        ...
        foreach($new_data as $key=>$row){
            if($key != 'updated_by' && $key != 'updated_by'){
                $old = $old_data[$key];
                $prev_dt[$key] = $old;
                $new_dt[$key] =  $row;
            }
